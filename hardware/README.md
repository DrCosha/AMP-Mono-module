...
Список элементов, примененных в схемах модуля CPU и платы его питания представлен в файле [BOM AMP CPU & PM.csv](https://github.com/DrCosha/AMP-Mono-module/blob/master/hardware/BOM_AMP_CPU_&_PM_rev4.csv), а так же в таблице ниже. Указанные элементы подобраны для использования с печатными платами, __GERBER__ файлы которых приведены в разделе ___hardware___.
...
Общее потребление платы контроллера и управляемых им реле от standby источника питания не превышает в пике 350mA. Мной применен трансформатор **PTC10** компании YHDC (www.yhdc.com) со следующими параметрами:
- мощность 10VA
- входное напряжение 2*115V, которые могут бытьвключены последовательно для обеспечения питания от 230V;
- выходное напряжение 2*12V, обмотки могут быть включены параллельно, для обеспечения максимального тока во вторичной обмотке.
